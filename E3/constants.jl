...
Final test
"""
recall_odds_threshold_final = 0.1^power_taken #recall odds threshold specifically for final test
x =0.13-0.1
cfinal_start=0.3#(0.09+x-0.000)^power_taken;
cfinal_end=0.3 #(0.08+x+0.06)^power_taken;
cfinal_rate = 0.27 #this value lower will make the tail of the F drop (and eliminate the final curvy bump)

# criterion_final = asym_decrease_shift_fj(cfinal_start, cfinal_start-cfinal_end, cfinal_rate, n_lists) #0.37
...
p_ListChange_finaltest =  0.0#13 #make this a const value rather than a vector
